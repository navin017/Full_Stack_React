...
function Quiz() {
	...
	const handleOptionSelection = (answerOption) =>{
		// const selectedIds = choosed.map(id => id.toString())
		fetch('/quiz',{
			method:'POST',
			headers:{'content-type':'application/json'},
			
			body: JSON.stringify({questionId: query[currentQuestion].id,
			selectedOption: answerOption,})
		})
		.then((response)=>response.json)
		.then((data)=>{
			console.log("Selected option stored to Database",data.message)
			history.push('/question2')
		})
		.catch((err)=>{
			console.log("Error Occured while fetching the Selected option",err)
		})
	}
									...
									{query[currentQuestion].answerOption.map((answerOption, options, index) => (
										...
										<ul className='unList' onClick={() => {setChoosed([answerOption.id].concat(choosed)); setStyle(options); scoreCard(answerOption.iscorrect) }}>
											<li className='list'
												...
												onClick={()=> handleOptionSelection(answerOption)}
												>
												{answerOption.answer}
											</li>
										</ul>
